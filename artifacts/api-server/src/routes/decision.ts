...
// =========================== EXPIRY RISK ===========================
// Inventory planning based on per-batch expiry dates captured on purchase lines.
// Uses average daily consumption (last 30d) to estimate whether the remaining
// shelf life is enough to consume the on-hand quantity before each batch expires.

router.get("/decision/inventory/expiry-risk", authMiddleware, async (req, res): Promise<void> => {
  const horizonDays = Math.max(7, Math.min(180, Number(req.query.horizonDays) || 30));
  const today = new Date();
  const todayStr = fmtDate(today);
  const consumptionWindow = 30;
  const fromConsumption = fmtDate(addDays(today, -consumptionWindow));

  // 1. Pull batches that have an expiry date and were not yet expired before today.
  const batches = await db.select({
    purchaseId: purchaseLinesTable.purchaseId,
    purchaseLineId: purchaseLinesTable.id,
    ingredientId: purchaseLinesTable.ingredientId,
    ...
    .from(purchaseLinesTable)
    .innerJoin(purchasesTable, eq(purchaseLinesTable.purchaseId, purchasesTable.id))
  ...
  if (batches.length === 0) {
    res.json({
      horizonDays,
      summary: { batchesTracked: 0, expiringSoon: 0, expiredOnHand: 0, valueAtRisk: 0 },
      batches: [],
      ingredientPlan: [],
      note: "No purchase batches with expiry dates yet. Add expiry on new purchases to activate planning.",
    });
    return;
  }

  // 2. Compute average daily consumption per ingredient over the last 30 days
  //    (theoretical, from sales × recipe). Falls back to 0 when no recipe usage.
  const salesItems = await db.select({
    menuItemId: salesInvoiceLinesTable.menuItemId,
    qty: sql<number>`COALESCE(SUM(${salesInvoiceLinesTable.quantity}), 0)`,
  })
    .from(salesInvoiceLinesTable)
    .innerJoin(salesInvoicesTable, eq(salesInvoiceLinesTable.invoiceId, salesInvoicesTable.id))
    .where(and(gte(salesInvoicesTable.salesDate, fromConsumption), lte(salesInvoicesTable.salesDate, todayStr)))
    .groupBy(salesInvoiceLinesTable.menuItemId);

  const recipes = await db.select().from(recipeLinesTable);
  const ingsForUsage = await db.select({
    id: ingredientsTable.id,
    conversionFactor: ingredientsTable.conversionFactor,
  }).from(ingredientsTable);
  const convMap = new Map(ingsForUsage.map(i => [i.id, i.conversionFactor || 1]));

  const ingredientUsage = new Map<number, number>(); // ingredientId -> total qty consumed in stock UOM
  for (const sale of salesItems) {
    const rs = recipes.filter(r => r.menuItemId === sale.menuItemId);
    for (const r of rs) {
      const conv = convMap.get(r.ingredientId) || 1;
      const netQty = (r.quantity || 0) * (1 + (r.wastagePercent || 0) / 100);
      const consumedStockQty = (Number(sale.qty) * netQty) / conv;
      ingredientUsage.set(r.ingredientId, (ingredientUsage.get(r.ingredientId) || 0) + consumedStockQty);
    }
  }

  const dailyUsage = new Map<number, number>();
  for (const [ingId, totalQty] of ingredientUsage.entries()) {
    dailyUsage.set(ingId, totalQty / consumptionWindow);
  }

  // 3. Per-batch risk scoring
  const batchRows = batches.map(b => {
    const days = b.expiryDate ? daysBetween(todayStr, b.expiryDate) : 999;
    const value = r2((b.quantity || 0) * (b.unitRate || 0));
    let status: "expired" | "critical" | "watch" | "ok";
    if (days < 0) status = "expired";
    else if (days <= 7) status = "critical";
    else if (days <= horizonDays) status = "watch";
    else status = "ok";
    return {
      ...
      uom: b.purchaseUom || b.stockUom || "unit",
      quantity: r2(b.quantity || 0),
      unitRate: r2(b.unitRate || 0),
      value,
  ...
  // 4. Per-ingredient consumption plan
  // For each ingredient, look at all non-expired batches sorted by expiry,
  // compute days needed to clear current stock at avg daily usage, and flag risk.
  const byIngredient = new Map<number, typeof batchRows>();
  for (const r of batchRows) {
    if (!byIngredient.has(r.ingredientId)) byIngredient.set(r.ingredientId, []);
    byIngredient.get(r.ingredientId)!.push(r);
  }

  const ingredients = await db.select().from(ingredientsTable);
  const ingredientPlan: any[] = [];
  for (const [ingId, ingBatches] of byIngredient.entries()) {
    const ing = ingredients.find(x => x.id === ingId);
    const live = ingBatches.filter(b => b.status !== "expired").sort((a, b) => (a.expiryDate || "").localeCompare(b.expiryDate || ""));
    if (live.length === 0) continue;
    const onHand = ing?.currentStock ?? 0;
    const avgDaily = dailyUsage.get(ingId) || 0;
    const daysOfCover = avgDaily > 0 ? Math.round(onHand / avgDaily) : null;
    const earliest = live[0];
    let recommendedAction = "Monitor";
    if (earliest.status === "critical") {
      if (avgDaily <= 0) recommendedAction = "Push usage urgently — no consumption history";
      else if (daysOfCover !== null && daysOfCover > earliest.daysToExpiry) {
        recommendedAction = `Use first or discount — ${daysOfCover}d cover but expires in ${earliest.daysToExpiry}d`;
      } else {
        recommendedAction = "Use first (FIFO)";
      }
    } else if (earliest.status === "watch") {
      recommendedAction = "Plan FIFO usage";
    }

    ingredientPlan.push({
      ingredientId: ingId,
      ingredientName: earliest.ingredientName,
      uom: ing?.stockUom || earliest.uom,
      onHandStock: r2(onHand),
      avgDailyUsage: r2(avgDaily),
      daysOfCover,
      nextExpiryDate: earliest.expiryDate,
      daysToNextExpiry: earliest.daysToExpiry,
      batchesAtRisk: live.filter(b => b.status === "critical" || b.status === "watch").length,
      valueAtRisk: r2(live.filter(b => b.status === "critical" || b.status === "watch").reduce((s, b) => s + b.value, 0)),
      recommendedAction,
    });
  }
  ingredientPlan.sort((a, b) => (a.daysToNextExpiry ?? 999) - (b.daysToNextExpiry ?? 999));

  const expiringSoon = batchRows.filter(b => b.status === "critical" || b.status === "watch");
  const expiredOnHand = batchRows.filter(b => b.status === "expired");
  const valueAtRisk = expiringSoon.reduce((s, b) => s + b.value, 0) + expiredOnHand.reduce((s, b) => s + b.value, 0);

  res.json({
    horizonDays,
    summary: {
      batchesTracked: batchRows.length,
      expiringSoon: expiringSoon.length,
      expiredOnHand: expiredOnHand.length,
      valueAtRisk: r2(valueAtRisk),
    },
    batches: batchRows
      .filter(b => b.status !== "ok")
      .sort((a, b) => a.daysToExpiry - b.daysToExpiry)
      .slice(0, 50),
    ingredientPlan: ingredientPlan.slice(0, 30),
  });
});
