...
- **Bulk Data Import**: Excel upload for sales invoices, purchases, expenses, menu items with recipes, ingredients, vendors, customers, and categories — all with row-by-row validation, name-matching, and downloadable templates. Petpooja sales no longer have a manual Excel import path — they flow exclusively through the live POS webhook (`POST /api/webhook/petpooja/:integrationId`), which auto-creates categories and menu items by name when they don't exist.
